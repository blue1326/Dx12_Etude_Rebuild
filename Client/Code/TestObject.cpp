...
HRESULT CTestObject::Init_GameObject(void)
{
	...
	cbvDesc.BufferLocation = dynamic_cast<CConstantBuffer<ObjectConstant>*>(pConstantBuffer.get())->Address();
	cbvDesc.SizeInBytes = dynamic_cast<CConstantBuffer<ObjectConstant>*>(pConstantBuffer.get())->Size();
...
int CTestObject::Update_GameObject(const std::shared_ptr<CTimer> t)
{
	...
	dynamic_cast<CConstantBuffer<ObjectConstant>*>(pConstantBuffer.get())->SetData(objConstants);
